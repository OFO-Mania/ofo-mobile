...
import {instantTopup} from '../../services/TransactionAPI';
...
const TopUpScreen = props => {
	...
	const doInstantTopUp = async () => {
		try {
			setLoading();
			setTopUpConfirmModal(false);
			const { data } = await instantTopup(accessToken, amount);
			setStatus(true);
			setNoRef(data.transaction.transaction_id);
			setMessage('Successfully top up.');
				...
				<Modal isVisible={topUpConfirmModal}>
					<View style={[StylePLN.containerModal, {height: 300}]}>
						...
						<View style={{flexDirection: 'row', justifyContent: 'center'}}>
							<TouchableOpacity
								style={[StylePLN.buttonCancel, {marginTop: 50, width: '45%'}]}
								onPress={() => setTopUpConfirmModal(false)}>
								<Text style={StylePLN.textCancel}>Cancel</Text>
							</TouchableOpacity>
							<TouchableOpacity
								style={[
									StylePLN.buttonConfirm,
									{marginTop: 50, width: '45%', marginLeft: 10},
								]}
								onPress={doInstantTopUp}>
								<Text style={StylePLN.textConfirm}>Confirm</Text>
							</TouchableOpacity>
						</View>
					</View>
				</Modal>
			...
			<View style={{flex: 1}}>
				<Modal isVisible={topUpModal}>
					<View style={[StylePLN.containerModal, {height: 400}]}>
						...
						<TouchableOpacity
							style={[StylePLN.buttonConfirm, {marginTop: 25}]}
							onPress={() => setTopUpModal(false)}>
							<Text style={StylePLN.textConfirm}>Okay</Text>
						</TouchableOpacity>
					</View>
				</Modal>
			</View>
